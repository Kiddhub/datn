...
const RoomChat = ({ roomChat, showAlert, setShowAlert, setAlertSeverity, setAlertMessage }) => {
  ...
  const sendMessage = async (data) => {
    ...
    try {
      ...
      const res = await postFetch('shop/chats/messages', form, token);
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '90vh', width: '870px' }}>
        ...
        <List>
          {messages?.map((message, index) => (
            <Box key={index} sx={{ marginBottom: '1rem' }}>
              {roomChat.shop?.id === message.senderId ? (
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', p: 2, gap: 1, width: '870px', justifyContent: 'space-between' }}>
        <TextField label="Type a message" sx={{ width: '800px' }} value={message} onChange={(e) => setMessage(e.target.value)} />
