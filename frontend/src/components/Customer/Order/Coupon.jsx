...
const Coupon = ({ couponOpen, couponClose, onApply,price}) => {
  ...
  const loadCoupons = async () => {
    try {
      const res = await getFetch(`/user/coupon/`,"")
      console.log("..",res)
      setCoupons(res)
    } catch (err) {
      console.error(err);
  ...
  const handleApplyCoupon = () => {
    ...
    couponClose(); // Close the modal after applying the coupon
  };
  ...
  return (
    ...
    >
      ...
      >
          ...
          <FormControl component="fieldset">
            ...
            >
              {coupons.map((coupon) => (
                <FormControlLabel
                  key={coupon.id}
                  disabled={coupon.minPrice > price}
        ...
        <Button onClick={couponClose} sx={{ mt: 2, marginLeft: 1 }}>
          Đóng Modal
        </Button>
      </Box>
    </Modal>
